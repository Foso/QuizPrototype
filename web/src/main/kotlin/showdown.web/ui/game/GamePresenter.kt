...
class GamePresenter(private val view: GameContract.View) : GameContract.Presenter {
    ...
    override fun onCreate() {
        ...
        gameDataSource.observeGameState().subscribe(onNext = { gameState ->
            when (gameState) {
                ...
                is GameState.TaskUpdate -> {
                    view.newState {
                        this.task= gameState.task
                    }
                }
            }
        }).addTo(compositeDisposable)
    ...
    override fun onBuzzerPressed() {
        gameDataSource.onBuzzerPressed()
    }

    override fun onAnswered(key: String) {
        gameDataSource.onAnswered(key)
    }
